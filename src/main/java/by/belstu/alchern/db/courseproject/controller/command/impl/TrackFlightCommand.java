...
public class TrackFlightCommand implements ActionCommand {
    @Override
...
//            if (user.getRole().getRole().equals(RequestParameterName.USER_ROLE_ADMIN)) {
        ...
        List<Flight> flightsOnTheWay = null;
        List<Flight> flightsDeparting = null;
        String orderBy = request.getParameter(RequestParameterName.REQ_PARAM_ORDER);
        try {
            flightsOnTheWay = flightService.getOnTheWay(orderBy);
            flightsDeparting = flightService.getDeparting();
        ...
        //change
        request.setAttribute(RequestParameterName.REQ_ATTR_PLANES_ON_THE_WAY, flightsOnTheWay);
        request.setAttribute(RequestParameterName.REQ_ATTR_PLANES_DEPARTING, flightsDeparting);

        page = ConfigurationManager.getProperty("path.page.trackflights");
//            } else {
//                // change
